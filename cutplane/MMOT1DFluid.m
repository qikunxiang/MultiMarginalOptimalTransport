classdef MMOT1DFluid < LSIPMinCuttingPlaneAlgo
    % Class for multi-marginal optimal transport (MMOT) problems that originated from the study of incompressible fluid where the 
    % marginals are all uniform on [0,1] and the cost function is given by f(x1, ... ,xN) = |x1 - x2|^2 + |x_2 - x_3|^2 + ... + 
    % |x{N-1} - xN|^2 + |PHI(x1) - xN|^2 and PHI() encodes the final arrangment of the particles
    ...
    properties(GetAccess = public, SetAccess = protected)
        ...
        % a constant term that was subtracted from the cost function due to not affecting the optimal coupling
        QuadraticTerm;
    end

    methods(Access = public)
        function obj = MMOT1DFluid(timestep_num, arrangement, testfuncs_knot_num, varargin)
            % Constructor method
            % Inputs:
            %   timestep_num: integer indicating the number of steps in the time discretization
            %   arrangment: cell array containing structs with fields knots and values encoding the arrangement of [0,1] into [0,1] via
            %   a continuous piece-wise function testfuncs_knot_num: the number of knots used in defining the simplicial test functions
            %   for the marginals
            ...
            if ~isfield(obj.Options, 'dual_prob_thres') || isempty(obj.Options.dual_prob_thres)
                obj.Options.dual_prob_thres = 1e-6;
            end
            ...
            if ~isfield(obj.LPOptions, 'FeasibilityTol') || isempty(obj.LPOptions.FeasibilityTol)
                obj.LPOptions.FeasibilityTol = 1e-9;
            end

            if ~isfield(obj.LPOptions, 'OptimalityTol') || isempty(obj.LPOptions.OptimalityTol)
            ...
            % set the arrangement
            % warning: we do not check if the continuous piece-wise affine function indeed maps [0, 1] to [0, 1]
            obj.Arrangement = struct;
            assert(length(arrangement.knots) == length(arrangement.values) && all(diff(arrangement.knots) > 0) ...
                && abs(min(arrangement.knots)) < eps && abs(max(arrangement.knots) - 1) < eps && abs(min(arrangement.values)) < eps ...
                && abs(max(arrangement.values) - 1) < eps, 'the arrangement function is mis-specified');
            ...
            % compute the constant term that corresponds to the integral of quadratic functions over the uniform marginal measures
            obj.QuadraticTerm = (2 * timestep_num - 1) / 3;
            obj.QuadraticTerm = obj.QuadraticTerm + sum(obj.Arrangement.KnotDiff .* (obj.Arrangement.ValueDiff .^ 2 / 3 ...
                + obj.Arrangement.ValueDiff .* obj.Arrangement.Values(1:end - 1) + obj.Arrangement.Values(1:end - 1) .^ 2));

            % Lipschitz constaints of the cost function with respect to each of the inputs
            arr_lip = max(abs(diff(arrangement.values) ./ diff(arrangement.knots)));
            obj.Arrangement.CostLipschitzConsts = 4 * ones(timestep_num, 1);
            obj.Arrangement.CostLipschitzConsts(1) = 2 * (1 + arr_lip);
            ...
            for marg_id = 1:timestep_num
                ...
                if marg_id == 1
                    % add the knots in the arrangement function into the test functions for the first marginal to simplify the global 
                    % minimization oracle 

                    marg_knots = [marg_knots; obj.Arrangement.Knots]; %#ok<AGROW>
        ...
        function vals = evaluateCostFunction(obj, inputs)
            % Evaluate the cost function at given input points. Note that the quadratic terms are not included
            % Inputs: 
            %   inputs: matrix where each row represents the coordinates of each input point
            % Output:
            %   vals: vector containing the computed cost values

            vals = sum(inputs(:, 1:end - 1) .* inputs(:, 2:end), 2);
            arranged = sum(min(max((inputs(:, 1) - obj.Arrangement.Knots(1:end - 1)') ./ obj.Arrangement.KnotDiff', 0), 1) ...
                .* obj.Arrangement.ValueDiff', 2) + obj.Arrangement.Values(1);
        ...
        function coup_indices = generateHeuristicCoupling(obj)
            % Heuristically couple the marginals by applying comonotone coupling of the marginals
            % Outputs:
            %   coup_indices: matrix containing the coupled knot indices in the marginals
            ...
            for marg_id = 1:marg_num
                atoms_cell{marg_id} = obj.Marginals{marg_id}.SimplicialTestFuncs.Knots;
                probs_cell{marg_id} = obj.Marginals{marg_id}.SimplicialTestFuncs.Integrals;
            end

            [coup_indices, ~] = comonotone_coupling(atoms_cell, probs_cell);
        end
        
        function coup_indices = updateSimplicialTestFuncs(obj, testfuncs_knot_num)
            ...
            % Input:
            %   testfuncs_knot_num: the number of knots used in defining the updated simplicial test functions for the marginals
            % Outputs:
            %   coup: matrix containing the indices of the coupled atoms in the discretized marginals
            ...
            % retrieve the truncated dual solution resulted from the cutting-plane algorithm
            dual_sol = obj.Runtime.DualSolution;
            
            old_coup_atoms = dual_sol.AtomsTruncated;
            old_coup_probs = dual_sol.ProbabilitiesTruncated;
            ...
            for marg_id = 1:marg_num
                ...
                new_marg_atoms_cell{marg_id} = obj.Marginals{marg_id}.SimplicialTestFuncs.Knots;
                new_marg_probs_cell{marg_id} = obj.Marginals{marg_id}.SimplicialTestFuncs.Integrals;
            end

            [coup_indices, ~] = discrete_reassembly_1D(old_coup_atoms, old_coup_probs, new_marg_atoms_cell, new_marg_probs_cell);
        ...
        function initializeSimplicialTestFuncs(obj)
            ...
            for marg_id = 1:marg_num
                ...
                % the coefficient corresponding to the first test function will not be included in the decision variable of the LP 
                % problem for identification purposes
                deci_logical_cell{marg_id} = [0; ones(testfunc_knot_num - 1, 1)];
            end

            obj.Storage.TotalKnotNum = sum(obj.Storage.MargKnotNumList);

            % compute the offset of the knots in the marginals in the vector containing all knots
            knot_num_cumsum = cumsum(obj.Storage.MargKnotNumList);
            obj.Storage.MargKnotNumOffsets = [0; knot_num_cumsum(1:end - 1)];

            % store the indices to place the decision variables of the LP problem in a vector containing the coefficients of the test 
            % functions
            obj.Storage.DeciVarIndicesInTestFuncs = find(vertcat(deci_logical_cell{:}));

            % concatenate the knots in all marginals into a single vector for the ease of retrieving the knot values
        ...
        function vals = evaluateMMOTDualFunctions(obj, pts, include_quad)
            ...
            % Inputs:
            %   pts: vector containing input points between 0 and 1 at which the dual functions need to be evaluated
            ...
            assert(all(pts >= 0 & pts <= 1), 'input points must be between 0 and 1');
            ...
            if include_quad
                quad_vals = pts.^2;
                arranged = sum(min(max((pts - obj.Arrangement.Knots(1:end - 1)') ./ obj.Arrangement.KnotDiff', 0), 1) ...
                .* obj.Arrangement.ValueDiff', 2) + obj.Arrangement.Values(1);
                vals(:, 1) = vals(:, 1) + quad_vals + arranged.^2;
                vals(:, 2:end) = vals(:, 2:end) + quad_vals * 2;
            end
        end
        ...
        function samps = randSampleFromOptCoupling(obj, samp_num, rand_stream)
            % Generate independent random sample from the optimized coupling of the marginals
            % Inputs: 
            %   samp_num: number of samples to generate
            %   rand_stream: RandStream object (default is RandStream.getGlobalStream)
            % Output:
            %   samps: matrix containing the samples from the coupling of the (continuous) uniform marginals
            %   samps_disc: matrix containing the indices of the samples from the coupling of the discretized uniform marginals
        ...
        function UB = getMMOTUpperBound(obj)
            % Compute an upper bound for the MMOT problem. The bound is computed directly based on the comonotone coupling and is not
        ...
        function outputs = evaluateOptMongeMaps(obj, marg1_inputs, batch_size)
            % Evaluate the computed approximately optimal Monge maps from inputs from the first marginal. Computation is done in
            % batches if necessary.
            % Inputs: 
            %   marg1_inputs: vector representing input points in [0, 1] from the first marginal
            %   batch_size: number of inputs to be used for each vectorized procedure (default is 1e4)
            % Output:
            %   outputs: matrix containing the evaluated Monge maps where each row represents an input point and each column represents
            %   a marginal
            ...
            for batch_id = 1:batch_num
                batch_inputs = marg1_inputs((batch_id - 1) * batch_size + 1:min(batch_id * batch_size, input_num));

                output_cell{batch_id} = obj.doEvaluateOptMongeMaps(batch_inputs, comonotone_map);
            end

            outputs = vertcat(output_cell{:});

            % add the final positions after the rearrangement into the last column of the output
            arranged = sum(min(max((marg1_inputs(:, 1) - obj.Arrangement.Knots(1:end - 1)') ./ obj.Arrangement.KnotDiff', 0), 1) ...
                .* obj.Arrangement.ValueDiff', 2) + obj.Arrangement.Values(1);

            outputs = [outputs, arranged];
        end

        function [UB, samps] = getMMOTUpperBoundMonteCarlo(obj, samp_num, rand_stream)
            % Compute an upper bound for the MMOT problem. The bound is approximated by Monte Carlo integration. 
            % Inputs: 
        ...
        function EB = getMTErrorBoundBasedOnOT(obj)
            % Compute the error bound for the objective value of the MMOT problem based on the Lipschitz constant of the cost functions
            % and the optimal transport distances between the marginals and their discretizations
            % Output:
            ...
            for marg_id = 1:marg_num
                ...
                EB = EB + obj.Arrangement.CostLipschitzConsts(marg_id) * marg.computeOTCost();
            end
        end
        ...
        function EB = getMTTheoreticalErrorBound(obj, LSIP_tolerance)
            ...
            for marg_id = 1:marg_num
                ...
                EB = EB + obj.Arrangement.CostLipschitzConsts(marg_id) * 2 * marg.SimplicialTestFuncs.MeshSize;
            end
        end

        function coup_indices = heuristicTimeBisection(obj)
            % Generate a coupling for a temporally refined version of the problem where each time step is divided in half. 
            % Output:
            %   coup_indices: matrix containing the indices of the coupled knots in the refined problem
            ...
            atoms_interpol = (atoms_coord(:, 1:end - 1) + atoms_coord(:, 2:end)) / 2;

            interp_indices_left = zeros(old_atom_num, marg_num - 1);

            for marg_id = 1:marg_num - 1
                interp_indices_left(:, marg_id) = min(old_atom_num - 1, sum((atoms_interpol(:, marg_id) - knots') >= 0, 2));
            end

            coup_indices = zeros(old_atom_num * 3, marg_num * 2 - 1);

            coup_indices(:, 1:2:marg_num * 2 - 1) = repmat(old_indices, 3, 1);
            coup_indices(1:old_atom_num, 2:2:marg_num * 2 - 2) = interp_indices_left;
            coup_indices(old_atom_num + 1:old_atom_num * 2, 2:2:marg_num * 2 - 2) = interp_indices_left + 1;
            coup_indices(old_atom_num * 2 + 1:end, 2:2:marg_num * 2 - 2) = old_indices(:, 2:end);
        end

        function setLSIPSolutions(obj, primal_sol, dual_sol, LSIP_UB, LSIP_LB)
        ...
        function setComonotoneMap(obj, comonotone_map)
            % Set the computed comonotone map in the runtime environment. This is to allow certain quantities to be computed using 
            % stored versions of the primal and dual solutions without executing the cutting-plane algorithm again. 
    ...
    methods(Access = protected)
        ...
        function prepareRuntime(obj)
            ...
            % initialize the starting node in the global minimization oracle to be 1
        ...
        function [UB, comono] = computeComonotoneUB(obj, atom_indices, probs)
            % Compute an upper bound for the MMOT problem via constructing a comonotone coupling from the dual LP relaxation. This 
            % upper bound does not require Monte Carlo integration.
            % Inputs:
            %   atom_indices: matrix containing the knot indices of the atoms in the discrete measure
            %   probs: probabilities of the atoms in the discrete measure
            % Outputs:
            %   UB: the computed upper bound
            %   comono: struct containing fields probabilities and map that represents the comonotone coupling computed from the input
            %   which also represents Monge maps from the first marginal to the rest
            ...
            [atom_indices, sorted_order] = sortrows(atom_indices, 1:marg_num);
            ...
            for marg_id = 1:marg_num
                ...
                % sort the marginal atoms into ascending order (note that there will be ties)
                [~, marg_atoms_order] = sort(atom_indices(:, marg_id), 'ascend');
                probs_cum = cumsum(probs(marg_atoms_order));
                comono.map(marg_atoms_order, marg_id) = [0; probs_cum(1:end - 1)];
            end

            % compute the upper bound by integrating along the diagonal lines (corresponding to the support of the comonotone coupling)
            UB = 0;
            ...
            for marg_id = 1:marg_num - 1
                ...
                UB = UB + sum(lowerlim_left .* lowerlim_right .* probs + (lowerlim_left + lowerlim_right) .* probs.^2 / 2 ...
                    + probs.^3 / 3);
            end
            ...
            marg1_arr = sum(min(max((marg1_map - obj.Arrangement.Knots(1:end - 1)') ./ obj.Arrangement.KnotDiff', 0), 1) ...
                .* obj.Arrangement.ValueDiff', 2) + obj.Arrangement.Values(1);

            % counter indicating the original intervals that have been processed
            counter_ori = 0;
            ...
            for knot_id = 2:arr_knot_num
                ...
                % process the left half of the interval (that is, the part of the interval to the left of the knot)
                ...
                if leftint_size >= 1e-10
                    % if the left half of the interval is very small (possibly due to rounding error), skip it
                    lowerlim_left(counter_new + 1) = margN_map(int_contain);
                    lowerlim_right(counter_new + 1) = marg1_arr(int_contain);
                ...
                if rightint_size < 1e-10 || knot_id == arr_knot_num
                    % if the right half of the interval is very small (possibly due to rounding error) or when the current knot is the 
                    % last one, skip it
                    continue;
                end

                % process the right half of the interval (that is, the part of the interval to the right of the knot)
                lowerlim_left(counter_new + 1) = margN_map(int_contain) + leftint_size;
            ...
            UB = UB + sum(lowerlim_left .* lowerlim_right .* probs_inc + (lowerlim_right + slopes .* lowerlim_left) ...
                .* probs_inc.^2 / 2 + slopes .* probs_inc.^3 / 3);
        ...
        function updateRuntimeAfterLP(obj, result)
            ...
            % the probabilities need to be normalized due to possible constraint violations from the LP solver
            probs = probs / sum(probs);

            [UB, comono] = obj.computeComonotoneUB(atom_indices, probs);

            if ~isfield(obj.Runtime, 'ComonotoneMap') || isempty(obj.Runtime.ComonotoneMap) || UB < obj.Runtime.ComonotoneMap.UB
                obj.Runtime.ComonotoneMap = struct('UB', UB, 'Info', comono);
            end
        end
        ...
        function [min_lb, optimizers] = callGlobalMinOracle(obj, vec)
            ...
            %   optimizers: cell array containing structs with fields inputs and qualites that correspond to the approximate optimizers
            %   of the global minimization problems
            ...
            % convert the locations of the knots and the coefficients in the decision variable into a cell array
            tf_vec = zeros(obj.Storage.TotalKnotNum, 1);
            tf_vec(obj.Storage.DeciVarIndicesInTestFuncs) = vec(2:end);
            tf_cell = cell(marg_num, 2);

            for marg_id = 1:marg_num
                tf_cell{marg_id, 1} = obj.Marginals{marg_id}.SimplicialTestFuncs.Knots;
                tf_cell{marg_id, 2} = tf_vec(obj.Storage.MargKnotNumOffsets(marg_id) + (1:(obj.Storage.MargKnotNumList(marg_id))));
            end

            % evaluate the arrangement function at all knots of marginal 1 including the added knots from the arrangment function 
            % itself
            arranged = sum(min(max((tf_cell{1, 1} - obj.Arrangement.Knots(1:end - 1)') ./ obj.Arrangement.KnotDiff', 0), 1) ...
                .* obj.Arrangement.ValueDiff', 2) + obj.Arrangement.Values(1);

            % begin the max-product algorithm
            % we choose a root node and perform variable elimination forward; for example, starting from marginal 1, we would perform 
            % elimination of marginals 2, 3, 4, ... and eventually maximize over marginals 1 and N; alternatively, starting from
            % marginal N-2, we would perform elimination of marginals N-1, N, 1, 2, ... and eventually maximize over marginals N-2 and
            % N-3
            ...
            % store the lookup tables for identifying the optimal states later
            ...
            % when eli_node is marginal 1, the locations of the knots need to pass through the arrangement function
            if eli_node == 1
                prod_s_e_right = arranged;
            end

            % in this cost matrix, each row corresponds to a state in start_node and each column corresponds to a state in eli_node
            cost_s_e_mat = (-2 * prod_s_e_left) .* prod_s_e_right';


            while mod(eli_node, marg_num) + 1 ~= start_node
                % this is the node that will be eliminated in the next iteration if it is not the starting node
                next_node = mod(eli_node, marg_num) + 1;

                next_state_num = length(tf_cell{next_node, 1});

                % to eliminate eli_node, we compute the cost associated with pair (eli_node, next_node)
                prod_e_n_left = tf_cell{eli_node, 1};
                prod_e_n_right = tf_cell{next_node, 1};

                % when next_node marginal 1, the locations of the knots need to pass through the arrangement function
                if next_node == 1
                ...
                % add the two cost matrices and subtract the test function values on eli_node
                cost_mat = repmat(cost_s_e_mat - tf_cell{eli_node, 2}', next_state_num, 1) ...
                    + repelem(cost_e_n_mat', start_state_num, 1);

                % compute the minimum value and the minimizer for each combination of states in start_node and states in next_node
                [min_cost_vec, min_state_vec] = min(cost_mat, [], 2);

                % store the minimizer states in the eliminated node
                lookup_table_cell{eli_node} = reshape(min_state_vec, start_state_num, next_state_num);

                % advance to the next node
                eli_node = next_node;
                cost_s_e_mat = reshape(min_cost_vec, start_state_num, next_state_num);
            end
            ...
            % when start_node is marginal 1, the locations of the knots need to pass through the arrangement function
            if start_node == 1
                prod_e_n_right = arranged;
            end
            ...
            % add the two cost matrices and subtract the test function values on eli_node; also add the constant term in the objecive
            cost_mat = cost_s_e_mat + cost_e_n_mat' - tf_cell{eli_node, 2}' - obj_const;

            [min_cost_vec, min_state_vec] = min(cost_mat, [], 2);

            % finally, subtract the test function values on start_node and minimize over the states in start_node
            ...
            % trace back the lookup tables to retrieve the minimizing states (knot indices)
            ...
            % the approximately optimal states in eli_node that was eliminated last
            min_knot_indices(:, eli_node) = min_state_vec(min_knot_indices(:, start_node));

            % trace from eli_node to all of the previously eliminated nodes in reverse direction

            while mod(eli_node - 2, marg_num) + 1 ~= start_node
                prev_node = mod(eli_node - 2, marg_num) + 1;

                min_knot_indices(:, prev_node) = lookup_table_cell{prev_node}(sub2ind(size(lookup_table_cell{prev_node}), ...
                    min_knot_indices(:, start_node), min_knot_indices(:, eli_node)));
            ...
            % every time the global minimization oracle is called, we change the value of the starting node to create more diverse
            % sub-optimal solutions
            obj.Runtime.GlobalMin.StartNode = mod(obj.Runtime.GlobalMin.StartNode, marg_num) + 1;
        end
        ...
        function updateLSIPUB(obj, min_lb, ~) 
            ...
            obj.Runtime.LSIP_UB = obj.Runtime.LSIP_LB - min_lb;
        end
        ...
        function addConstraints(obj, optimizers)
            ...
            col_indices = optimizers.knot_indices' + obj.Storage.MargKnotNumOffsets;
            ...
            A_full = sparse(repelem((1:constr_num)', marg_num, 1), col_indices(:), 1, constr_num, obj.Storage.TotalKnotNum);
            ...
            obj.Runtime.CutIndices = [obj.Runtime.CutIndices; optimizers.knot_indices];
        ...
        function primal_sol = buildPrimalSolution(obj, result, violation)
            ...
            for marg_id = 1:marg_num
                primal_sol.Coefficients{marg_id} = knot_coefs(obj.Storage.MargKnotNumOffsets(marg_id) ...
        ...
        function dual_sol = buildDualSolution(obj, result)
            ...
            dual_sol = struct;
            pos_list = result.pi > 0;
            [dual_indices, ~, umap] = unique(obj.Runtime.CutIndices(pos_list, :), 'rows', 'stable');
            dual_probs = accumarray(umap, result.pi(pos_list), [size(dual_indices, 1), 1]);

            % store every atom with positive weight
            dual_sol.Probabilities = dual_probs;
            dual_sol.Indices = dual_indices;
            dual_sol.Atoms = obj.Storage.MargKnotsConcat(dual_sol.Indices + obj.Storage.MargKnotNumOffsets');

            % normalize the probabilities since there can be numerical errors from the LP solver
            dual_sol.Probabilities = dual_sol.Probabilities / sum(dual_sol.Probabilities);

            % also store the truncated version where atoms with probabilities below the threshold are discarded
            dual_keep_list = dual_probs > obj.Options.dual_prob_thres;
            dual_sol.ProbabilitiesTruncated = dual_probs(dual_keep_list);
            dual_sol.ProbabilitiesTruncated = dual_sol.ProbabilitiesTruncated / sum(dual_sol.ProbabilitiesTruncated);
            dual_sol.IndicesTruncated = dual_sol.Indices(dual_keep_list, :);
            dual_sol.AtomsTruncated = dual_sol.Atoms(dual_keep_list, :);
        end

        function samps = doRandSampleFromReassembly(obj, samp_num, rand_stream)
            ...
            for marg_id = 1:marg_num
                disc_marg_atoms = obj.Marginals{marg_id}.SimplicialTestFuncs.Knots;
                disc_marg_probs = obj.Marginals{marg_id}.SimplicialTestFuncs.Integrals;

                obj.Marginals{marg_id}.setCoupledDiscreteMeasure(disc_marg_atoms, disc_marg_probs);
            end
            ...
            knot_indices_samps = obj.Runtime.DualSolution.Indices(disc_atom_index_samps, :);
            ...
            for marg_id = 1:marg_num
                ...
                atom_num_list = accumarray(marg_knot_indices_samps, 1, [obj.Storage.MargKnotNumList(marg_id), 1]);

                % generate from the conditional distributions
                cont_samp_cell = marg.conditionalRandSample((1:obj.Storage.MargKnotNumList(marg_id))', atom_num_list, rand_stream);

                % fill in the coupled samples from the continuous marginals
                for atom_id = 1:length(atom_num_list)
                    samps(marg_knot_indices_samps == atom_id, marg_id) = cont_samp_cell{atom_id};
                end
            end
        end

        function outputs = doEvaluateOptMongeMaps(~, marg1_inputs, comonotone_map)
            % Evaluate sub-optimal Monge maps from inputs from the first 
            % marginal. 
            % Inputs: 
            %   marg1_inputs: vector representing input points in [0, 1] from the first marginal
            %   comonotone_map: matrix representing Monge maps where each row corresponds to an interval and each column corresponds
            %   to a marginal
            % Output:
            %   outputs: matrix containing the evaluated Monge maps where each row represents an input point and each column represents
            %   a marginal
            ...
            interval_indices = sum(marg1_inputs >= comonotone_map(:, 1)', 2);

            % location of each input point in its respective sub-interval with respect to the left end point
            interval_location = marg1_inputs - comonotone_map(interval_indices, 1);

            % select the left end points of the respective sub-intervals for each marginal and then add the increment terms
            outputs = comonotone_map(interval_indices, :) + interval_location;
        end
        ...
        function display_string = buildMessage(obj)
            ...
            display_string = sprintf(['%s: iteration %4d: LSIP_LB = %10.4f, LSIP_UB = %10.4f, diff = %10.6f, ' ...
                'MMOT_LB = %10.4f, MMOT_UB = %10.4f, diff = %10.6f\n'], ...
                class(obj), ...
                obj.Runtime.iter, ...
                obj.Runtime.LSIP_LB, ...
                obj.Runtime.LSIP_UB, ...
