classdef MT2DQuad_MMOT < MT2DQuad
    ...
    methods(Access = public)
        function obj = MT2DQuad_MMOT(marginals, weights, quality_cell, ...
            ...
            % the global minimization oracle can be formulated as grid
            % branch-and-bound (GridBnB) or mixed-integer quadratic program
            % (MIQP)
            if ~isfield(obj.Options, 'global_formulation') ...
                    || isempty(obj.Options.global_formulation)
                obj.Options.global_formulation = 'GridBnB';
            end

            % set the default options for the global minimization oracle
            if strcmp(obj.Options.global_formulation, 'GridBnB')
            ...
            % initialize the global formulation
            if strcmp(obj.Options.global_formulation, 'GridBnB')
                obj.initializeGlobalGridBnB();
            elseif strcmp(obj.Options.global_formulation, 'MIQP')
                obj.initializeGlobalMIQP();
            else
        ...
        function initializeSimplicialTestFuncs(obj)
            ...
            if strcmp(obj.Options.global_formulation, 'GridBnB')
                obj.initializeSimplicialTestFuncsGridBnB();
            elseif strcmp(obj.Options.global_formulation, 'MIQP')
                obj.initializeSimplicialTestFuncsMIQP();
            end
    ...
    methods(Access = protected)
        ...
        function initializeGlobalGridBnB(obj)
            % Initialization of the global minimization oracle when the
            % global formulation is grid branch-and-bound
            ...
            for poly_id = 1:poly_num
                vertices = obj.Quality.Polytopes{poly_id};
        ...
        function initializeGlobalMIQP(obj)
            % Initialization of the global minimization oracle when the
            % global formulation is mixed-integer quadratic programming
        ...
        function initializeSimplicialTestFuncsGridBnB(obj)
            % Initialize some quantities related to the simplicial test 
            % functions of the marginals when the global formulation is
            % grid branch-and bound
        ...
        function initializeSimplicialTestFuncsMIQP(obj)
            % Initialize some quantities related to the simplicial test 
            % functions of the marginals when the global formulation is
            ...
            % compute the variable indices
            % the variable in the quality space
            z_indices = (1:2)';
            z_lb = -inf(2, 1);
            z_ub = inf(2, 1);
            z_vtype = repmat('C', 2, 1);
            var_counter = 2;

            % the variables representing the values of the convex functions
            % (the max part)
            q_indices = var_counter + (1:marg_num)';
            q_lb = -inf(marg_num, 1);
            q_ub = inf(marg_num, 1);
            q_vtype = repmat('C', marg_num, 1);
            var_counter = var_counter + marg_num;

            % the variables representing the differences from the maximums
            sigma_indices = cell(marg_num, 1);
            total_vert_num = obj.Storage.TotalVertNum;
            
            for marg_id = 1:marg_num
                sigma_indices{marg_id} = var_counter ...
                    + (1:obj.Storage.MargVertNumList(marg_id))';
                var_counter = var_counter ...
                    + obj.Storage.MargVertNumList(marg_id);
            end
            
            sigma_lb = zeros(total_vert_num, 1);
            sigma_ub = inf(total_vert_num, 1);
            sigma_vtype = repmat('C', total_vert_num, 1);

            % the binary variables enforcing one term to attain the maximum
            % for each marginal
            ...
            for marg_id = 1:marg_num
                iota_indices{marg_id} = var_counter ...
                    + (1:obj.Storage.MargVertNumList(marg_id))';
                marg_indices_cell{marg_id} = ...
                    (1:obj.Storage.MargVertNumList(marg_id))';
                var_counter = var_counter ...
                    + obj.Storage.MargVertNumList(marg_id);
            end

            iota_lb = -inf(total_vert_num, 1);
            iota_ub = inf(total_vert_num, 1);
            iota_vtype = repmat('B', total_vert_num, 1);

            var_length = var_counter;

            % sparse matrix for extracting the optimal quality values from
            % computed solutions
            quality_extract_mat = sparse([1; 2], z_indices, 1, ...
                2, var_length);

            % sparse matrix for extracting the optimal vertex indices from
            % computed solutions
            indices_extract_mat = sparse( ...
                repelem((1:marg_num)', obj.Storage.MargVertNumList, 1), ...
                vertcat(iota_indices{:}), ...
                vertcat(marg_indices_cell{:}), ...
                marg_num, var_length);

            % build the constraint matrices
            constr_counter = 0;

            % the constraints that enforces the q variables to be equal the
            % maximums
            A_max_cell = cell(marg_num, 1);
            rhs_max_cell = cell(marg_num, 1);
            sense_max_cell = cell(marg_num, 1);

            % vectors that store the indices in the RHS of the constraints
            % where the coefficients of the simplicial test functions
            % should be placed (subject to a minus sign)
            rhs_coef_indices = cell(marg_num, 1);

            % row and column indices in the constraint matrix where the
            % big M values should be placed
            A_bigM_indices = cell(marg_num, 2);

            % vectors that store the indices in the RHS of the constraints
            % where the big M values should be placed
            rhs_bigM_indices = cell(marg_num, 1);

            for marg_id = 1:marg_num
                marg = obj.Marginals{marg_id};
                marg_vert = marg.SimplicialTestFuncs.Vertices';
                marg_vert_num = size(marg_vert, 2);

                % the constraints that link the q variables with the sigma
                % variables
                A_link = sparse([repelem((1:marg_vert_num)', 2, 1); ...
                    (1:marg_vert_num)'; ...
                    (1:marg_vert_num)'], ...
                    [repmat(z_indices, marg_vert_num, 1); ...
                    sigma_indices{marg_id}; ...
                    q_indices(marg_id) * ones(marg_vert_num, 1)], ...
                    [marg_vert(:) * 2 * obj.MarginalWeights(marg_id); ...
                    ones(marg_vert_num, 1); ...
                    -ones(marg_vert_num, 1)], ...
                    marg_vert_num, var_length);
                rhs_link = zeros(marg_vert_num, 1);
                sense_link = repmat('=', marg_vert_num, 1);
                
                rhs_coef_indices{marg_id} = constr_counter ...
                    + (1:marg_vert_num)';
                constr_counter = constr_counter + marg_vert_num;

                % the constraints that forces one of the sigma variables to
                % be equal to 0
                A_zero = sparse((1:marg_vert_num)', ...
                    sigma_indices{marg_id}, 1, marg_vert_num, var_length);
                rhs_zero = zeros(marg_vert_num, 1);
                sense_zero = repmat('<', marg_vert_num, 1);

                A_bigM_indices{marg_id, 1} = constr_counter + ...
                    (1:marg_vert_num)';
                A_bigM_indices{marg_id, 2} = iota_indices{marg_id};
                rhs_bigM_indices{marg_id} = constr_counter + ...
                    (1:marg_vert_num)';

                constr_counter = constr_counter + marg_vert_num;


                % the constraint that requires all iota variables to sum to
                % 1
                A_sum = sparse(ones(marg_vert_num, 1), ...
                    iota_indices{marg_id}, 1, 1, var_length);
                rhs_sum = 1;
                sense_sum = '=';
                constr_counter = constr_counter + 1;

                A_max_cell{marg_id} = [A_link; A_zero; A_sum];
                rhs_max_cell{marg_id} = [rhs_link; rhs_zero; rhs_sum];
                sense_max_cell{marg_id} = [sense_link; sense_zero; ...
                    sense_sum];
            end

            % the constraints characterizing each polytope in the quality
            % space (note that one MIQP model needs to be built for each
            % polytope)
            poly_num = length(obj.Quality.Polytopes);
            A_quality_cell = cell(poly_num, 1);
            rhs_quality_cell = cell(poly_num, 1);
            sense_quality_cell = cell(poly_num, 1);

            for poly_id = 1:poly_num
                poly_w = obj.Quality.HyperplaneConcise.w{poly_id};
                poly_b = obj.Quality.HyperplaneConcise.b{poly_id};
                poly_ineq_num = length(poly_b);

                A_quality_cell{poly_id} = sparse( ...
                    repmat((1:poly_ineq_num)', 2, 1), ...
                    repelem(z_indices, poly_ineq_num, 1), ...
                    poly_w(:), ...
                    poly_ineq_num, var_length);
                rhs_quality_cell{poly_id} = poly_b;
                sense_quality_cell{poly_id} = repmat('<', ...
                    poly_ineq_num, 1);
            end

            % build the gurobi models
            obj.Storage.GlobalMin.GurobiModels = cell(poly_num, 1);

            for poly_id = 1:poly_num
                model = struct;
                model.modelsense = 'min';
                model.objcon = 0;
                model.obj = zeros(var_length, 1);
                model.obj(q_indices) = -1;
                model.Q = sparse(z_indices, z_indices, 1, ...
                    var_length, var_length);
    
                model.lb = [z_lb; q_lb; sigma_lb; iota_lb];
                model.ub = [z_ub; q_ub; sigma_ub; iota_ub];
                model.vtype = [z_vtype; q_vtype; sigma_vtype; iota_vtype];

                model.A = [vertcat(A_max_cell{:}); ...
                    A_quality_cell{poly_id}];
                model.rhs = [vertcat(rhs_max_cell{:}); ...
                    rhs_quality_cell{poly_id}];
                model.sense = [vertcat(sense_max_cell{:}); ...
                    sense_quality_cell{poly_id}];
    
                obj.Storage.GlobalMin.GurobiModels{poly_id} = model;
            end

            obj.Storage.GlobalMin.RHSCoefLinkMat = ...
                -speye(obj.Storage.TotalVertNum);
            obj.Storage.GlobalMin.RHSCoefIndices = ...
                vertcat(rhs_coef_indices{:});
            obj.Storage.GlobalMin.MatBigMIndices = ...
                [vertcat(A_bigM_indices{:, 1}), ...
                vertcat(A_bigM_indices{:, 2})];
            obj.Storage.GlobalMin.RHSBigMIndices = ...
                vertcat(rhs_bigM_indices{:});

            obj.Storage.GlobalMin.QualityExtractMat = quality_extract_mat;
            obj.Storage.GlobalMin.IndicesExtractMat = indices_extract_mat;
        end
        ...
        function [min_lb, optimizers] = callGlobalMinOracle(obj, vec)
            ...
            if strcmp(obj.Options.global_formulation, 'GridBnB')
                ...
                [pool_inds, pool_pts, pool_vals, LB] ...
                    = obj.computeGlobalMinGridBnB(vert_vals, ...
                    objective_const);
            elseif strcmp(obj.Options.global_formulation, 'MIQP')
                % call the gurobi MIQP solver
                [pool_inds, pool_pts, pool_vals, LB] ...
                    = obj.computeGlobalMinMIQP(vert_vals, ...
                    objective_const);
            else
                error('unknown formulation of global minimization');
            end
        ...
        function [pool_inds, pool_pts, pool_vals, LB] ...
                = computeGlobalMinGridBnB(obj, vert_vals, objective_const)
        ...
        function [pool_inds, pool_pts, pool_vals, LB] ...
                = computeGlobalMinMIQP(obj, vert_vals, objective_const)
            % Approximately solve the global minimization problem via the
            % MIQP solver of gurobi
            % Inputs:
            %   vert_vals: values of the simplicial test functions at the
            %   vertices concatenated into a vector
            %   objective_const: constant part of the objective function
            ...
            %   global minimization problem in the form of vertex indices
            %   in the triangulations for each of the input measures; the
            %   algorithm will retain up to obj.GlobalOptions.PoolSolutions
            %   approximate optimizers
            %   pool_pts: two-column matrix containing the approximate
            %   optimizers in the two-dimensional space; this will be used
            %   to generate linear inequality constraints
            ...
            marg_num = length(obj.MarginalWeights);
            poly_num = length(obj.Quality.Polytopes);
            inds_cell = cell(poly_num, 1);
            pts_cell = cell(poly_num, 1);
            vals_cell = cell(poly_num, 1);
            LB_list = zeros(poly_num, 1);

            % compute the big M values
            bigM_list = zeros(marg_num, 1);
            coef_counter = 0;
            ...
            for marg_id = 1:marg_num
                ...
                bigM_list(marg_id) = ...
                    ...
                    * 2 * obj.MarginalWeights(marg_id) ...
                    + range(vert_vals(coef_counter ...
                    + (1:marg_vert_num)));
                coef_counter = coef_counter + marg_vert_num;
            end

            gm = obj.Storage.GlobalMin;

            for poly_id = 1:poly_num
                model = gm.GurobiModels{poly_id};

                % fill in the constant objective
                model.objcon = -objective_const;

                % fill the RHS of the constraints with the coefficients of
                % the simplicial test functions
                model.rhs(gm.RHSCoefIndices) = gm.RHSCoefLinkMat ...
                    * vert_vals;

                % fill the constraint matrix with the big M values
                bigM_expanded = repelem(bigM_list, ...
                    obj.Storage.MargVertNumList);
                linind = sub2ind(size(model.A), ...
                    gm.MatBigMIndices(:, 1), gm.MatBigMIndices(:, 2));
                model.A(linind) = bigM_expanded;
                
                % fill the RHS of the constraints with the big M values
                model.rhs(gm.RHSBigMIndices) = bigM_expanded;

                % invoke the gurobi MIQP solver
                ...
                LB_list(poly_id) = result.objbound;

                % get a set of approximate optimizers
                if isfield(result, 'pool')
                    opts = horzcat(result.pool.xn)';
                    opt_vals = [result.pool.objval]';
                else
                    opts = result.x';
                    opt_vals = result.objval;
                end

                pts_cell{poly_id} = opts * gm.QualityExtractMat';
                inds_cell{poly_id} = round(opts * gm.IndicesExtractMat');
                vals_cell{poly_id} = opt_vals;
            end

            pts = vertcat(pts_cell{:});
            inds = vertcat(inds_cell{:});
            vals = vertcat(vals_cell{:});

            [~, sorted_ind] = sort(vals, 'ascend');
            cand_ind = sorted_ind(1:min(gl_options.PoolSolutions, ...
                length(sorted_ind)));

            pool_inds = inds(cand_ind, :);
            pool_pts = pts(cand_ind, :);
            pool_vals = vals(cand_ind);
            LB = min(LB_list);
        end
        ...
        function addConstraints(obj, optimizers)
            ...
            for marg_id = 1:marg_num
                tf = obj.Marginals{marg_id}.SimplicialTestFuncs;
                rhs_new = rhs_new ...
                    - (2 * obj.MarginalWeights(marg_id)) ...
                    * sum(optimizers.points .* ...
                    tf.Vertices(optimizers.vertex_indices(:, ...
                    marg_id), :), 2);
            end
