...
def Uniform_sample_generator(rng, batch_size):
    while 1:
        yield rng.random([batch_size, NUM_OF_MARGS, DIM])

# Build tf graph
def costfunc_tf(x):
    if ARRANGEMENT == 1:
        return -OT_Fluid_cost_tf(x, OT_Fluid_arrangement1_tf)
    elif ARRANGEMENT == 2:
        return -OT_Fluid_cost_tf(x, OT_Fluid_arrangement2_tf)


def costfunc_np(x):
    if ARRANGEMENT == 1:
        return -OT_Fluid_cost_np(x, OT_Fluid_arrangement1_np)
    elif ARRANGEMENT == 2:
        return -OT_Fluid_cost_np(x, OT_Fluid_arrangement2_np)
...
num_of_runs = 20
save_list = [NUM_OF_MARGS, PENALTY_TYPE, PENALTY_PARAM_GAMMA, ACTIVATION, NUM_OF_LAYERS, NUM_OF_NEURONS_PER_LAYER]
save_list_string = [str(x) for x in save_list]

filename_prefix = 'Saved/Fluid/NN/arr' + str(ARRANGEMENT) + '/' + '_'.join(save_list_string)
...
for run_i in range(num_of_runs):
    rng_training = np.random.RandomState(80000 + 10000 * ARRANGEMENT + 100 * NUM_OF_MARGS + 10 * run_i)
    rng_sampling_dual = np.random.default_rng(180000 + 10000 * ARRANGEMENT + 100 * NUM_OF_MARGS + 10 * run_i)
    rng_sampling_primal_prop = np.random.default_rng(280000 + 10000 * ARRANGEMENT + 100 * NUM_OF_MARGS + 10 * run_i)
    rng_sampling_primal_acc = np.random.default_rng(380000 + 10000 * ARRANGEMENT + 100 * NUM_OF_MARGS + 10 * run_i)
    t0 = time.time()


    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        marg_sample_generator = Uniform_sample_generator(rng_training, BATCH_MARGS)
        refjoint_sample_generator = Uniform_sample_generator(rng_training, BATCH_REFJOINT)
        ...
        final_dual_sample_generator = Uniform_sample_generator(rng_sampling_dual, NUM_OF_MC_SAMP_FINAL_OBJ)
        ...
        if COMPUTE_PRIMAL:
            ...
            final_primal_sample_generator = Uniform_sample_generator(rng_sampling_primal_prop, NUM_OF_MC_SAMP_FINAL_OBJ)
