...
COMPARISONS_OURALGO_TIMESTEP_NUM_INDEX = 1;
COMPARISONS_OURALGO_TESTFUNC_NUM_INDICES = [4, 6];
...
COMPARISONS_LEGEND = vertcat({'RKHS'; 'NN-dual'; 'NN-primal'}, COMPARISONS_OURALGO_LEGEND);

COMPARISONS_SAMPLE_NUM = 20;

COMPARISONS_RKHS_FILEPREFIX = 'RKHS/';
COMPARISONS_RKHS_FILESUFFIX = '_Laplace_0.5_exp_objectives.txt';
...
COMPARISONS_NN_FILESUFFIX_PRIMAL = '_ReLu_5_64_objectives_primal.txt';
...
COMPARISONS_NN_DUAL_COLUMN_NUM = 4;
COMPARISONS_NN_PRIMAL_COLUMN_NUM = 4;

COMPARISONS_TOTAL_COLUMN_NUM = COMPARISONS_RKHS_COLUMN_NUM + COMPARISONS_NN_DUAL_COLUMN_NUM + COMPARISONS_NN_PRIMAL_COLUMN_NUM;
COMPARISONS_GROUPING = [ones(COMPARISONS_RKHS_COLUMN_NUM, 1); ...
    2 * ones(COMPARISONS_NN_DUAL_COLUMN_NUM, 1); ...
    3 * ones(COMPARISONS_NN_PRIMAL_COLUMN_NUM, 1)];
...
for arr_id = 1:length(arrangements)
    ...
    % primal objectives computed by the NN-based algorithm
    for test_id = 1:length(COMPARISONS_NN_FILENAMES_CELL{arr_id})
        txt_filename = [CONFIG.COMPARISONPATH, COMPARISONS_NN_FILEPREFIX, ...
            COMPARISONS_NN_FILENAMES_CELL{arr_id}{test_id}, COMPARISONS_NN_FILESUFFIX_PRIMAL];
...
ha = tight_subplot(2, 1, [0.006, 0], [0.035, 0.040], [0.070, 0.004]);
for arr_id = 1:length(arrangements)
    axes(ha(arr_id));
    ...
    set(gca, 'XTick', [0.666, 1.666, 3:6, (7:10) + 0.333]);
    set(gca, 'YGrid', 'on');
