...
for arr_id = 1:arrangment_num
    ...
    for timestep_id = 1:timestep_test_num
        for testfunc_id = 1:testfunc_test_num
            if testfunc_id == 1
                OT = MMOT1DFluid(timestep_num_list(timestep_id), ...
                    arrangements{arr_id}, ...
                    testfunc_knot_num_list(testfunc_id), ...
                    options, LP_options, global_options);
    
                coup_indices = OT.generateHeuristicCoupling();
            else
                coup_indices = OT.updateSimplicialTestFuncs( ...
                    testfunc_knot_num_list(testfunc_id));
            end
            
            init_constr = struct('knot_indices', coup_indices);
            
            output_cell{arr_id}{timestep_id, testfunc_id} = ...
                OT.run(init_constr, tolerance);
            ...
            LSIP_LB_cell{arr_id}(timestep_id, testfunc_id) = ...
                OT.Runtime.LSIP_LB;
            LSIP_UB_cell{arr_id}(timestep_id, testfunc_id) = ...
                OT.Runtime.LSIP_UB;
            LSIP_primal_cell{arr_id}{timestep_id, testfunc_id} = ...
                OT.Runtime.PrimalSolution;
            LSIP_dual_cell{arr_id}{timestep_id, testfunc_id} = ...
                OT.Runtime.DualSolution;
            OT_comonotone_map_cell{arr_id}{timestep_id, testfunc_id} = ...
                OT.Runtime.ComonotoneMap;
        
            log_text = sprintf(['arrangement %1d, %2d time steps, ' ...
                '%3d knots: ' ...
                'LB = %8.4f, UB = %8.4f, ' ...
                'diff = %10.6f, THEB = %10.6f\n'], ...
                arr_id, timestep_num_list(timestep_id), ...
                testfunc_knot_num_list(testfunc_id), ...
                OT_LB, OT_UB, ...
                OT_UB - OT_LB, THEB);
