classdef ProbMeas2DAffDens < ProbMeas2DConvexPolytopeWithW2OT ...
    ...
    methods(Access = public)
        function obj = ProbMeas2DAffDens(vertices, dens_weight, ...
                dens_intercept)
            ...
            for tri_id = 1:tri_num
                tri_verts = obj.Supp.TriangleVertices( ...
                    obj.Supp.Triangles(tri_id, :), :);
    ...
    methods(Access = protected)
        function inside = doCheckIfInsideSupport(obj, pts)
            ...
            inside = all(pts * obj.Supp.Hyperplane.w' ...
                ...
                ProbMeas2DConvexPolytope.INSIDE_TOLERANCE, 2);
        end
        ...
        function [int1, int2] = computeW2InnerIntegral(obj, ...
            ...
            %   triangles in the triangulation of the support; not useful
            %   here since the density is globally affine
            % Outputs:
            ...
            % since the density is affine on the entire support, it can be
            % directly evaluated using the x- and y-coordinates
            dens_vertices = tri_vertices_x * obj.Dens.Weight(1) ...
                + tri_vertices_y * obj.Dens.Weight(2) ...
                + obj.Dens.Intercept;
        ...
        function postProcessW2OptimalTransport(obj)
            ...
            for atom_id = 1:atom_num
                power_cell_vertex_indices = output_triangulation( ...
                    power_cell_indices == atom_id, :);
                power_cell_vertices = output_vertices( ...
                    power_cell_vertex_indices, :);

                % store the bounding box of the power cell as well as the
                % polyshape representation of the power cell
                info = struct;
                info.BoxBottomLeft = min(power_cell_vertices, [], 1)';
                info.BoxWidthHeight = max(power_cell_vertices, [], 1)' ...
                    - info.BoxBottomLeft;
                ch = convhull(power_cell_vertices, 'Simplify', true);
                info.PolyShape = polyshape(power_cell_vertices(ch, :), ...
                    'Simplify', false, 'KeepCollinearPoints', false);

                % maximum density in the power cell
                max_density = max(power_cell_vertices ...
                    * obj.Dens.Weight + obj.Dens.Intercept) * 1.01 ...
                    / obj.W2OT.DiscMeas.Probs(atom_id);

                info.MaxDensity = max_density;
                info.Multiplier = max_density ...
                    * info.BoxWidthHeight(1) * info.BoxWidthHeight(2);
        ...
        function samps = condRejSampFromW2Coupling(obj, atom_id, ...
            ...
            batch_samp_num = min(ceil(samp_num ...
                * condrejsamp.Multiplier * 1.5), 1e4);
            ...
            while samp_counter < samp_num
                % raw samples before the rejection step
                raw_samps = condrejsamp.BoxWidthHeight' ...
                    .* rand(rand_stream, batch_samp_num, 2) ...
                    + condrejsamp.BoxBottomLeft';

                % compute the acceptance probabilities
                raw_density = (raw_samps * obj.Dens.Weight ...
                    + obj.Dens.Intercept) .* isinterior( ...
                    condrejsamp.PolyShape, raw_samps);

                accept_probs = raw_density ...
                    / (obj.W2OT.DiscMeas.Probs(atom_id) ...
                    * condrejsamp.Multiplier ...
                    / (condrejsamp.BoxWidthHeight(1) ...
                    * condrejsamp.BoxWidthHeight(2)));
    ...
    methods(Static, Access = public)
        ...
        function vals = affineIntegralTriangle(dens_vertices, ...
            ...
            % Inputs:
            %   dens_vertices: three-column matrix containing the values of
            %   the affine density function evaluated at the three vertices
            %   of the triangle
            %   vertices_x: three-column matrix containing the
            %   x-coordinates of the three vertices
            %   vertices_y: three-column matrix containing the
            %   y-coordinates of the three vertices
            %   polynomial_coefs: six-column matrix containing the
            %   coefficients in the integrand polynomial: f(x, y) = 
            %   a1 * x^2 + a2 * y^2 + a3 * x * y + a4 * x + a5 * y + a6
            %   after_transform: logical vector indicating whether the
            %   coefficients of each integrand polynomial is applicable to
            %   the variables before transformation or after transforming
            %   to the triangle with vertices [0; 0], [1; 0], and [1; 1]
            % Output: 
            %   vals: vector containing the values of the integrals

            % express the affine transformation:
            % [u;v] -> S([u;v]) = [S11, S12; S21, S22] * [u;v] + [S13; S23]
            % that satisfies S([0;0]) = [x1;y1], S([1;0]) = [x2;y2], and
            % S([1;1]) = [x3;y3]
            ...
            % compute the coefficients c1, c2, c3, c4, c5, c6 such that
            % f(S(x, y)) = c1 * x^2 + c2 * y^2 + c3 * x * y + c4 * x 
            % + c5 * y + c6
            ...
            c3 = 2 * a1 .* S11 .* S12 + a3 .* (S12 .* S21 + S11 .* S22) ...
                + 2 * a2 .* S21 .* S22;
            c4 = a4 .* S11 + a5 .* S21 + S13 .* (2 * a1 .* S11 ...
                + a3 .* S21) + S23 .* (2 * a2 .* S21 + a3 .* S11);
            c5 = a4 .* S12 + a5 .* S22 + S13 .* (2 * a1 .* S12 ...
                + a3 .* S22) + S23 .* (2 * a2 .* S22 + a3 .* S12);
            c6 = a1 .* S13.^2 + a3 .* S13 .* S23 + a2 .* S23.^2 ...
                + a4 .* S13 + a5 .* S23 + a6;

            % if after_transform is false, the coefficients do not need to
            % be updated
            ...
            vals = ((c1 .* pdiff21 ...
                + (c1 .* pdiff32 + c3 .* pdiff21) / 2 ...
                + (c3 .* pdiff32 + c2 .* pdiff21) / 3 ...
                + (c2 .* pdiff32) / 4) / 5 ...
                + ((c1 .* p1 + c4 .* pdiff21) ...
                + (c3 .* p1 + c4 .* pdiff32 ...
                + c5 .* pdiff21) / 2 ...
                + (c2 .* p1 + c5 .* pdiff32) / 3) / 4 ...
                + ((c4 .* p1 + c6 .* pdiff21) ...
                + (c5 .* p1 + c6 .* pdiff32) / 2) / 3 ...
                + (c6 .* p1) / 2) ...
                .* abs_det_term;
        end
    end
