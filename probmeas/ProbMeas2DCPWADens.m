classdef ProbMeas2DCPWADens < ProbMeas2DConvexPolytopeWithW2OT ...
    ...
    methods(Access = public)
        function obj = ProbMeas2DCPWADens(vertices, triangles, ...
                dens_vertices)
        ...
        function inside_mat = checkIfInsideTriangularRegion(obj, pts)
            ...
            region_ineq_mat = obj.Dens.EdgeIneq.w * pts' ...
                - obj.Dens.EdgeIneq.b <= ...
                ProbMeas2DConvexPolytope.INSIDE_TOLERANCE;
        ...
        function pts_san = sanitizePoints(obj, pts, tolerance)
            ...
            region_num = size(obj.Supp.Triangles, 1);
            input_num = size(pts, 1);
            constr_vio = obj.Dens.EdgeIneq.w * pts' ...
                - obj.Dens.EdgeIneq.b;

            vio_mat = reshape(max(reshape(constr_vio(:), ...
                3, region_num * input_num), [], 1)', ...
                region_num, input_num)';
        ...
        function info = saveW2OptimalTransportInfo(obj)
            ...
            info = saveW2OptimalTransportInfo@ ...
                ProbMeas2DConvexPolytopeWithW2OT(obj);
        ...
        function loadW2OptimalTransportInfo(obj, info)
            ...
            loadW2OptimalTransportInfo@ ...
                ProbMeas2DConvexPolytopeWithW2OT(obj, info);
    ...
    methods(Access = protected)
        ...
        function prepareOptimalTransport(obj, atoms, probs, angle_list)
            ...
            prepareOptimalTransport@ProbMeas2DConvexPolytope(obj, ...
        ...
        function [val1, val2] = computeInnerIntegral(obj, ...
            ...
            itg_ub = min(radial.interval_ub(:, angle_indices), ...
                upper_limits');
        ...
        function postProcessOptimalTransport(obj, pp_angle_indices)
            ...
            for atom_id = 1:atom_num
                [rho_temp, hyp_full] = ProbMeas2DConvexPolytope ...
        ...
        function [int1, int2] = computeW2InnerIntegral(obj, ...
                vertices, triangles, centers, mesh_indices)
            % Compute integrals of constant 1 functions as well as the
            % squared distance functions (from given centers) within
            % triangles with respect to the probability measure
            % Inputs:
            %   vertices: two-column matrix containing the vertices in the
            %   triangulation
            %   triangles: three-column matrix encoding the triangulation
            %   centers: two-column matrix containing centers with respect
            %   to which the second integral is evaluated
            %   mesh_indices: vector containing the indices of the
            %   triangles in the triangulation of the support; used to
            %   speed up the computation of the density
            % Outputs:
            %   int1: vector containing the computed integrals of the
            %   constant 1 function within each triangle; each component
            %   corresponds to a triangle
            %   int2: vector containing the computed integrals of the
            %   quadratic function (x, y) -> (x - c_x)^2 + (y - c_y)^2,
            %   where (c_x, c_y) are the coordinates of the center; each
            %   component corresponds to a triangle
            ...
            % the x- and y-coordinates of the three vertices of the
            % triangles
            tri_vertices_x = vertices_x(triangles')';
            tri_vertices_y = vertices_y(triangles')';

            % since the density is piece-wise affine, the weights and
            % intercepts depend on the mesh indices
            interp_mat = obj.Dens.InterpMat(mesh_indices, :);
            dens_vertices = tri_vertices_x .* interp_mat(:, 1) ...
                + tri_vertices_y .* interp_mat(:, 2) ...
                + interp_mat(:, 3);
        ...
        function postProcessW2OptimalTransport(obj)
            % Post-processing after computing semi-discrete Wasserstein-2 
            % optimal transport including computing the contour of each
            % cell in the Laguerre diagram and preparing for conditional
            % rejection sampling 

            atom_num = size(obj.W2OT.DiscMeas.Atoms, 1);
            mesh_triangle_num = size(obj.Supp.Triangles, 1);

            % shift the weights to make them all positive so that they can 
            % be interpreted as squared radii of circles in a power diagram
            squared_radii = obj.W2OT.Weights - min(obj.W2OT.Weights) + 1;

            % invoke the mex function to compute a triangular mesh formed
            % by the intersections of power cells and 
            [output_vertices, output_triangulation, power_cell_indices, ...
                mesh_indices] = mesh_intersect_power_diagram( ...
                obj.Supp.TriangleVertices, obj.Supp.Triangles, ...
                obj.W2OT.DiscMeas.Atoms, squared_radii, ...
            ...
            % compute the probabilities contained in each triangle in the
            % output mesh
            tri_probs = ProbMeas2DAffDens.affineIntegralTriangle( ...
                dens_vertices, tri_vertices_x, tri_vertices_y, ...
                repmat([0, 0, 0, 0, 0, 1], triangle_num, 1), ...
            ...
            for atom_id = 1:atom_num
                % first create as many sub-cells as the number of triangles
                % in the density mesh
                subcell_probs = zeros(mesh_triangle_num, 1);
                subcells = cell(mesh_triangle_num, 1);

                % keep track of the number of triangles with non-empty
                % intersection with the power cell
                mesh_triangle_counter = 0;

                for mesh_triangle_id = 1:mesh_triangle_num
                    tri_intersect_list = power_cell_indices == atom_id ...
                        & mesh_indices == mesh_triangle_id;
                    ...
                    % gather the triangles that are in the intersection
                    % of the power cell and the triangle and take their
                    % union (which is convex) to form the sub-cell
                    intersect_indices = output_triangulation( ...
                        tri_intersect_list, :);
                    intersect_vertices = output_vertices( ...
                        intersect_indices(:), :);
                    ch = convhull(intersect_vertices, ...
                        'Simplify', true);
                    ps = polyshape(intersect_vertices(ch, :), ...
                        'Simplify', false, ...
                        'KeepCollinearPoints', false);

                    % compute the total probability within this
                    % sub-cell
                    subcell_probs(mesh_triangle_counter) = ...
                        sum(tri_probs(tri_intersect_list));

                    info = struct;

                    % store information about the shape of this
                    % sub-cell
                    info.BoxBottomLeft = min(intersect_vertices, ...
                        [], 1)';
                    info.BoxWidthHeight = max(intersect_vertices, ...
                        [], 1)' - info.BoxBottomLeft;
                    info.PolyShape = ps;

                    % store information about the density inside the
                    % sub-cell
                    info.DensityWeightVec = ...
                        obj.Dens.InterpMat(mesh_triangle_id, 1:2)';
                    info.DensityIntercept = ...
                        obj.Dens.InterpMat(mesh_triangle_id, 3);

                    % store information about the multiplier in the
                    % rejection sampling
                    max_density = max(intersect_vertices ...
                        * info.DensityWeightVec ...
                        + info.DensityIntercept) * 1.01 ...
                        / subcell_probs(mesh_triangle_counter);

                    info.MaxDensity = max_density;
                    info.Multiplier = max_density ...
                        * info.BoxWidthHeight(1) ...
                        * info.BoxWidthHeight(2);

                    subcells{mesh_triangle_counter} = info;
                end

                obj.W2OT.CondRejSamp{atom_id} = struct;
                obj.W2OT.CondRejSamp{atom_id}.SubCells = ...
                    subcells(1:mesh_triangle_counter, :);
                obj.W2OT.CondRejSamp{atom_id}.SubCellProbabilities ...
                    = subcell_probs(1:mesh_triangle_counter);
            end
        end

        function samps = condRejSampFromW2Coupling(obj, atom_id, ...
                samp_num, rand_stream)
            % Perform rejection sampling of the conditional distribution
            % given an atom in the discrete measure; i.e., perform
            % rejection sampling within a power cell.
            % Inputs:
            %   atom_id: atom index
            ...
            subcell_probs = obj.W2OT.CondRejSamp{atom_id}...
                .SubCellProbabilities;
            subcell_num = length(subcells);

            % first sample the sub-cell indices; since we are sampling
            % conditional to the power cell, the sub-cell probabilities
            % need to be normalized
            subcell_index_samps = randsample(rand_stream, ...
                subcell_num, samp_num, true, ...
                subcell_probs / sum(subcell_probs));

            % count the number of samples for each cell
            subcell_samp_num_list = accumarray(subcell_index_samps, ...
                ones(samp_num, 1), [subcell_num, 1]);
            ...
            for subcell_id = 1:subcell_num
                ...
                % equal to 150% of samp_num times the
                % expected number of iterations for each sample, capped at
                % 1e4
                batch_samp_num = min(ceil(subcell_samp_num ...
                    * subcell_info.Multiplier * 1.5), 1e4);
    
                samp_counter = 0;
    
                while samp_counter < subcell_samp_num
                    % raw samples before the rejection step
                    subcell_raw_samps = subcell_info.BoxWidthHeight' ...
                        .* rand(rand_stream, batch_samp_num, 2) ...
                        + subcell_info.BoxBottomLeft';
    
                    % compute the acceptance probabilities
                    raw_density = (subcell_raw_samps ...
                        * subcell_info.DensityWeightVec ...
                        + subcell_info.DensityIntercept) ...
                        .* isinterior(subcell_info.PolyShape, ...
                        subcell_raw_samps);
    
                    accept_probs = raw_density ...
                        / (subcell_probs(subcell_id) ...
                        * subcell_info.Multiplier ...
                        / (subcell_info.BoxWidthHeight(1) ...
                        * subcell_info.BoxWidthHeight(2)));
                    ...
                    new_samp_num = min(acc_num, subcell_samp_num ...
                        - samp_counter);
    
                    samp_cell{subcell_id}(samp_counter ...
                        + (1:new_samp_num), :) ...
