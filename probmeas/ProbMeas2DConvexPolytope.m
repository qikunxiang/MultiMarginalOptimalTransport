classdef (Abstract) ProbMeas2DConvexPolytope < handle
    ...
    methods(Access = public)
        ...
        function [x, y, rho] = computeLaguerreDiagram(obj, ...
            ...
            lin = ProbMeas2DConvexPolytope.linePreprocess( ...
            ...
            for anchor_id = 1:anchor_num
                rho(:, anchor_id) = ProbMeas2DConvexPolytope ...
    ...
    methods(Static, Access = public)
        function [rho, hyp_full, lin_full] ...
                ...
                hyp_a, hyp_c, hyp_intercept, lin_c, lin_intercept, ...
                full_info)
            % Express a region in hyperbolic Dirichlet tessellation in the
            % polar coordinate where the origin is a center. The region is
            % bound by hyperbolas and straight lines. The function returns
            % the distance from the origin for the given inputs theta in
            % radian. The parameters have been preprocessed in the sense
            % that after rotation by hyp_intercept, a hyperbola will be in
            % its canonical orientation, and that after rotation by
            % lin_intercept, a straight line will be parallel to the x-axis
            % above the origin.
            % Inputs:
            %   theta: the vector-valued input in radian
            %   hyp_a: signed a-values of the hyperbolas (a positive value
            %   indicates the left branch and a negative value indicates
            %   the right branch) 
            %   hyp_c: c-values of the hyperbolas (i.e., distances from
            %   other centers divided by 2)
            %   hyp_intercept: rotation angles (in radian) to set the
            %   hyperbolas into their canonical orientation
            %   lin_c: distances of the straight lines from the origin
            %   lin_intercept: rotation angles (in radian) to set the
            %   straight lines to be parallel to the x-axis and above the
            %   origin 
            %   full_info: boolean indicating whether to return the full
            %   distance information in matrices (default is false)
            % Outputs:
            %   rho: the distances from the origin along the boundary of
            %   the region 
            %   hyp_full: matrix containing full distance information about
            %   boundaries formed by hyperbolas (equal to [] if full_info =
            %   false) 
            %   lin_full: matrix containing full distance information about
            %   boundaires formed by hyperplanes (equal to [] if full_info
            %   = false) 
            ...
            if any(hyp_b_sq <= 0 & hyp_a > 0)
                % if c < a, then the set is empty; if c = a, then the set
                % is a line, which is negligible
                ...
                hyp_rho = hyp_b_sq(list)' ./ (hyp_a(list)' ...
                    - hyp_c(list)' .* cos(theta - hyp_intercept(list)'));
    ...
    methods(Access = protected)
        function prepareOptimalTransport(obj, atoms, probs, angle_list)
            ...
            % prepare information related to the hyperbolas
            obj.OT.Prep.hyp = ...
                ProbMeas2DConvexPolytope.hyperbolaPreprocess(atoms);

            % prepare information related to the linear boundaries
            obj.OT.Prep.lin = ProbMeas2DConvexPolytope.linePreprocess( ...
        ...
        function [val, grad] = evaluateOTMinObjective(obj, OT_weights, ...
            ...
            for atom_id = 1:atom_num
                for batch_id = 1:batch_num
                    ...
                    upper_limits = ProbMeas2DConvexPolytope ...
                        .hyperbolic_Dirichlet_tessellation_polar( ...
