classdef (Abstract) ProbMeas2DConvexPolytopeWithW2OT ...
    ...
    % Abstract class for 2-dimensional probability measures supported
    % within a convex polytope which allows for the computation of optimal
    % transport with respect to the squared Euclidean distance, i.e.,
    % Wasserstein-2 optimal transport. The abstract methods can be
    % implemented for continuous probability measures. 

    properties(SetAccess = protected, GetAccess = public)
        % struct storing information about the Wasserstein-2 optimal
        % transference plan and the way to sample from the conditional
        % measure 
        W2OT = struct;
    end
    ...
    methods(Access = public)
        ...
        function [OT_weights, OT_cost, OT_output] ...
                = computeW2OptimalTransport(obj, atoms, probs, ...
                init_weights, optim_options)
            ...
            if OT_output.exitflag == 1
                ...
                error(['Error encountered during the computation ' ...
                    'of Wasserstein-2 optimal transport']);
            end
        ...
        function info = saveW2OptimalTransportInfo(obj)
            ...
            info.bounding_box.bottom_left = ...
                obj.W2OT.BoundingBox.BottomLeft;
            info.bounding_box.top_right = ...
        ...
        function loadW2OptimalTransportInfo(obj, info)
            ...
            obj.W2OT.DiscMeas = struct('Atoms', info.atoms, ...
                'Probs', info.probs);
        ...
        function testW2OTObjective(obj, atoms, probs, ...
                OT_weights, grid_pts_x, grid_pts_y)
            % Test the objective and gradient in the computation of
            % semi-discrete Wasserstein-2 optimal transport by comparing
            % with two-dimensional integration over a grid
            % Inputs: 
            %   atoms: two-column matrix containing the locations of the
            %   atoms in the discrete measure
            %   probs: vector containing the probabilities of the atoms in
            ...
            [val, grad] = obj.evaluateW2OTMinObjective(OT_weights(2:end));
            ...
            weighted_sqdist = (grid_x(:) - atoms(:, 1)') .^ 2 ...
                + (grid_y(:) - atoms(:, 2)') .^ 2 - OT_weights';
            [min_weighted_sqdist, min_id] = min(weighted_sqdist, [], 2);

            val_grid = -(OT_weights' * probs) - min_weighted_sqdist' ...
                * grid_density;
        ...
        function testW2OTGradient(obj, atoms, probs, OT_weights, nudge)
            ...
            % computation of semi-discrete Wasserstein-2 optimal transport
            % Inputs: 
            %   atoms: two-column matrix containing the locations of the
            %   atoms in the discrete measure
            %   probs: vector containing the probabilities of the atoms in
            ...
            for comp_id = 2:atom_num
                ...
                val_nudged = obj.evaluateW2OTMinObjective( ...
        ...
        function samp_cell = conditionalRandSampleFromW2Coupling(obj, ...
                samp_num_list, rand_stream)
            % Randomly generate samples from the conditional distributions
            % given the Wasserstein-2 optimally coupled atoms in the
            % discrete measure 
            % Inputs: 
            ...
            for atom_id = 1:atom_num
                ...
                % sampling
                samp_cell{atom_id} = obj.condRejSampFromW2Coupling( ...
                    atom_id, samp_num_list(atom_id), rand_stream);
            end
        end

        function [samps, disc_samps] = randSampleW2Coupled(obj, ...
                samp_num, rand_stream)
            % Randomly generate samples from the Wasserstein-2 optimal
            % coupling of the probability measure and the discrete measure
            % Inputs: 
            ...
            if ~isfield(obj.W2OT, 'Coupled') || ~obj.W2OT.Coupled
                error(['must set the Wasserstein-2 coupled ' ...
                    'discrete measure first']);
            end
        ...
        function [power_cell_list, power_cell_triangle_list, ...
                power_cell_indices, mesh_indices] = ...
                computePowerDiagram(obj, anchors, weights)
            % Compute the contour of each cell in the power diagram with
            % given anchors and weights
            % Inputs: 
            %   anchors: two-column matrix containing the locations of the
            %   anchor points
            %   weights: weights in the power diagram
            % Outputs:
            %   power_cell_list: vector containing polyshape objects
            %   describing the power cells
            %   power_cell_triangle_list: vector containing polyshape
            %   objects describing triangles in the triangulation of the
            %   power cell intersection with the density mesh
            %   power_cell_indices: vector containing the indices of power
            %   cells that correspond to the triangles in the power cell
            %   triangulation
            %   mesh_indices: vector containing the indices of mesh
            %   triangles that correspond to the the triangles in the power
            %   cell triangulation

            anchor_num = size(anchors, 1);

            bbox_bottomleft = min(min(obj.Supp.TriangleVertices, [], 1), ...
                min(anchors, [], 1))';
            bbox_topright = max(max(obj.Supp.TriangleVertices, [], 1), ...
                max(anchors, [], 1))';
            ...
            [output_vertices, output_triangulation, power_cell_indices, ...
                ...
                anchors, squared_radii, bbox_bottomleft, bbox_topright);

            triangle_num = size(output_triangulation, 1);
            triangle_cell = cell(triangle_num, 1);

            for triangle_id = 1:triangle_num
                cell_vertices = output_vertices( ...
                    output_triangulation(triangle_id, :), :);
            ...
            for anchor_id = 1:anchor_num
                poly_cell{anchor_id} = union(power_cell_triangle_list( ...
                    power_cell_indices == anchor_id), ...
    ...
    methods(Access = protected)
        ...
        function prepareW2OptimalTransport(obj, atoms, probs)
            ...
            obj.W2OT.BoundingBox = struct( ...
                'BottomLeft', min(min(atoms, [], 1), ...
                min(obj.Supp.TriangleVertices, [], 1))', ...
                'TopRight', max(max(atoms, [], 1), ...
                max(obj.Supp.TriangleVertices, [], 1))');
        end
        ...
        function [val, grad] = evaluateW2OTMinObjective(obj, OT_weights)
            ...
            %   corresponding to the dual weights in the power diagram (the
            %   first weight is set to 0 for identification purpose, and 
            %   thus the input here corresponds to the second weight 
            %   onwards)
            % Outputs:
            ...
            % compute a matrix containing the centers of the power cells as
            % rows
            power_cell_centers = obj.W2OT.DiscMeas.Atoms( ...
                power_cell_indices, :);

            [int1, int2] = obj.computeW2InnerIntegral(output_vertices, ...
                output_triangulation, power_cell_centers, mesh_indices);

            % regroup the integral values based on the power cell indices;
            % this evaluates the probability within each power cell
            int1 = accumarray(power_cell_indices, int1, ...
                [length(OT_weights), 1]);

            val = -(OT_weights' * obj.W2OT.DiscMeas.Probs) ...
    ...
    methods(Abstract, Access = protected)
        % Compute integrals of constant 1 functions as well as the squared
        % distance functions (from given centers) within triangles with
        % respect to the probability measure
        [int1, int2] = computeW2InnerIntegral(obj, vertices, triangles, ...
            centers, mesh_indices);

        % Post-processing after computing semi-discrete Wasserstein-2 
        % optimal transport including computing the contour of each cell in 
        % the Laguerre diagram and preparing for conditional rejection
        % sampling 
        postProcessW2OptimalTransport(obj);

        % Perform rejection sampling of the conditional distribution
        samps = condRejSampFromW2Coupling(obj, atom_id, ...
                samp_num, rand_stream);
    end
end
